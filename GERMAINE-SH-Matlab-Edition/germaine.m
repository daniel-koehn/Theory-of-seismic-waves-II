% GERMAINE Matlab-Edition - 2D SH FDFD seismic modelling code
% 
% Daniel Koehn
% Kiel, the 23rd of September 2016
...
% define FD-scheme
% FD_SCHEME = 1 - 9-point mixed grid stencil with PMLs
FD_SCHEME = 1;
...
if(model.MODEL==1)
    ...
    [vp,rho] = create_mod(model);
end

% extend model
if(model.npml>0)
    [vp,rho,model] = extend_model(model,vp,rho);
end
...
if(FD_SCHEME==1)
    ...
    PMLs = PML(model,vp,rho);
    A = init_A_SH_9p_mixed_pml(model,rho,PMLs);
end
...
set(get(cbar_handle,'ylabel'),'string','Vs [m/s]','fontsize',model.FSize,'FontWeight',model.Fweight);
...
colormap(seismic);
cbar_handle=colorbar('EastOutside');
set(get(cbar_handle,'ylabel'),'string','Re(uy)','fontsize',model.FSize,'FontWeight',model.Fweight);
...
xlabel('Distance [km]');
ylabel('Depth [km]');
title('Real part of uy wavefield');
